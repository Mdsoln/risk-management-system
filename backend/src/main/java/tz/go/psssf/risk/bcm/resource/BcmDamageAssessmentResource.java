...
import tz.go.psssf.risk.bcm.service.BcmDamageAssessmentService;
...
@Path("/api/v1/bcm/damage-assessment")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class BcmDamageAssessmentResource {

    @Inject
    Logger log;

    @Inject
    BcmDamageAssessmentService service;

    private static final DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.ISO_DATE_TIME;

    @GET
    @Path("/list")
    public ResponseWrapper<PaginatedResponse<BcmDamageAssessmentPojo>> listAll(
    ...
    @GET
    @Path("/{id}")
    public ResponseWrapper<BcmDamageAssessmentPojo> getById(@PathParam("id") String id) {
    ...
    @PUT
    @Path("/{id}")
    public ResponseWrapper<BcmDamageAssessmentPojo> update(@PathParam("id") String id, @Valid BcmDamageAssessmentDTO dto) {
