...
import tz.go.psssf.risk.bcm.service.BcmSystemListingService;
...
@Path("/api/v1/bcm/system-listing")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class BcmSystemListingResource {

    @Inject
    Logger log;

    @Inject
    BcmSystemListingService service;

    private static final DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.ISO_DATE_TIME;

    @GET
    @Path("/list")
    public ResponseWrapper<PaginatedResponse<BcmSystemListingPojo>> listAll(
    ...
    @GET
    @Path("/{id}")
    public ResponseWrapper<BcmSystemListingPojo> getById(@PathParam("id") String id) {
    ...
    @PUT
    @Path("/{id}")
    public ResponseWrapper<BcmSystemListingPojo> update(@PathParam("id") String id, @Valid BcmSystemListingDTO dto) {
