...
import tz.go.psssf.risk.compliance.service.ComplianceEntityCategoryService;
...
@Path("/api/v1/compliance/entity-category")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class ComplianceEntityCategoryResource {

    @Inject
    ComplianceEntityCategoryService service;

    private static final DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.ISO_DATE_TIME;

    @GET
    @Path("/list")
    public ResponseWrapper<PaginatedResponse<ComplianceEntityCategoryPojo>> listAll(
    ...
    @GET
    @Path("/{id}")
    public ResponseWrapper<ComplianceEntityCategoryPojo> getById(@PathParam("id") String id) {
    ...
    @PUT
    @Path("/{id}")
    public ResponseWrapper<ComplianceEntityCategoryPojo> update(@PathParam("id") String id, @Valid ComplianceEntityCategoryDTO dto) {
