...
import tz.go.psssf.risk.compliance.service.RegulatoryComplianceMatrixSectionService;
...
@Path("/api/v1/compliance/matrix/section")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class RegulatoryComplianceMatrixSectionResource {

    @Inject
    Logger log;

    @Inject
    RegulatoryComplianceMatrixSectionService service;

    private static final DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.ISO_DATE_TIME;

    @GET
    @Path("/list")
    public ResponseWrapper<PaginatedResponse<RegulatoryComplianceMatrixSectionPojo>> listAll(
    ...
    @GET
    @Path("/{id}")
    public ResponseWrapper<RegulatoryComplianceMatrixSectionPojo> getById(@PathParam("id") String id) {
    ...
    @PUT
    @Path("/{id}")
    public ResponseWrapper<RegulatoryComplianceMatrixSectionPojo> update(
            @PathParam("id") String id, @Valid RegulatoryComplianceMatrixSectionDTO dto) {
