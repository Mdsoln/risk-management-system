...
import tz.go.psssf.risk.bcm.service.BcmStaffService;
...
@Path("/api/v1/bcm/staff")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class BcmStaffResource {

    @Inject
    Logger log;

    @Inject
    BcmStaffService service;

    private static final DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.ISO_DATE_TIME;

    @GET
    @Path("/list")
    public ResponseWrapper<PaginatedResponse<BcmStaffPojo>> listAll(
    ...
    @GET
    @Path("/{id}")
    public ResponseWrapper<BcmStaffPojo> getById(@PathParam("id") String id) {
    ...
    @PUT
    @Path("/{id}")
    public ResponseWrapper<BcmStaffPojo> update(@PathParam("id") String id, @Valid BcmStaffDTO dto) {
