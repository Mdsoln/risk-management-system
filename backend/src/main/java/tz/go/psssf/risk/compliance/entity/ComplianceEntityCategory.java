...
@Audited
@Entity
@Table(name = "compliance_entity_category", uniqueConstraints = @UniqueConstraint(columnNames = {"code"}))
//@EntityListeners(RiskEntityListener.class)
//@FilterDef(name = "activeFilter", parameters = @ParamDef(name = "status", type = String.class))
public class ComplianceEntityCategory extends BaseEntity {
...
//    @Filter(name = "activeFilter", condition = "status = :status")
    private List<ComplianceEntity> entities = new ArrayList<>();
}