...
import tz.go.psssf.risk.compliance.mapper.ComplianceDocumentCategoryMapper;
...
@ApplicationScoped
public class ComplianceDocumentCategoryService {

    @Inject
    Logger log;

    @Inject
    ComplianceDocumentCategoryRepository repository;

    @Inject
    ComplianceDocumentCategoryMapper mapper;
    ...
    public ResponseWrapper<PaginatedResponse<ComplianceDocumentCategoryPojo>> findWithPaginationSortingFiltering(
        ...
        try {
            CustomQueryHelper<ComplianceDocumentCategory> customQueryHelper = new CustomQueryHelper<>();
            PanacheQuery<ComplianceDocumentCategory> query = customQueryHelper.findWithPaginationSortingFiltering(
                    repository,
                    ComplianceDocumentCategory.class, page, size, sort, sortDirection, searchKeyword,
                    SEARCHABLE_FIELDS, startDate, endDate, filterDateBy, VALID_SORT_FIELDS, VALID_DATE_FIELDS,
                    SORT_FIELD_MAPPINGS);

            PaginatedResponse<ComplianceDocumentCategory> paginatedResponse = PaginationHelper.toPageInfo(query);

            List<ComplianceDocumentCategoryPojo> pojos = paginatedResponse.getItems().stream()
                    .map(mapper::toPojo)
                    .collect(Collectors.toList());

            PaginatedResponse<ComplianceDocumentCategoryPojo> pojoPaginatedResponse = new PaginatedResponse<>(
    ...
    public ResponseWrapper<ComplianceDocumentCategoryPojo> findById(String id) {
        try {
            ComplianceDocumentCategory entity = repository.findById(id);
            if (entity == null) {
                return ResponseHelper.createNotFoundResponse();
            }
            return ResponseHelper.createSuccessResponse(mapper.toPojo(entity));
        } catch (Exception e) {
            log.error("Error finding ComplianceDocumentCategory by ID", e);
            return ResponseHelper.createErrorResponse(e);
        }
    }
    ...
    public ResponseWrapper<ComplianceDocumentCategoryPojo> create(@Valid ComplianceDocumentCategoryDTO dto) {
        try {
            ...
            ComplianceDocumentCategory entity = mapper.toEntity(dto);
            ...
            System.out.println("###############################################");
            System.out.println("###############################################");
            System.out.println(" getId "+   String.valueOf(entity.getId()));
            System.out.println(" getCode "+   String.valueOf(entity.getCode()));
            System.out.println(" getName "+   String.valueOf(entity.getName()));
            System.out.println(" getDescription "+   String.valueOf(entity.getDescription()));
            System.out.println(" getDocuments "+   String.valueOf(entity.getDocuments()));
            ...
            log.error("Error creating ComplianceDocumentCategory", e);
            return ResponseHelper.createErrorResponse(e);
        }
    }

    @Transactional
    public ResponseWrapper<ComplianceDocumentCategoryPojo> update(String id, @Valid ComplianceDocumentCategoryDTO dto) {
        try {
            ComplianceDocumentCategory entity = repository.findById(id);
            if (entity == null) {
                return ResponseHelper.createNotFoundResponse();
            ...
            log.error("Error updating ComplianceDocumentCategory", e);
            return ResponseHelper.createErrorResponse(e);
        }
    }
    ...
    public ResponseWrapper<Void> delete(String id) {
        try {
            ...
            log.error("Error deleting ComplianceDocumentCategory", e);
            return ResponseHelper.createErrorResponse(e);
        }
    }
}
