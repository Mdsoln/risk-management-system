...
import tz.go.psssf.risk.bcm.service.BcmImpactAssessmentService;
...
@Path("/api/v1/bcm/impact-assessment")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class BcmImpactAssessmentResource {

    @Inject
    Logger log;

    @Inject
    BcmImpactAssessmentService service;

    private static final DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.ISO_DATE_TIME;

    @GET
    @Path("/list")
    public ResponseWrapper<PaginatedResponse<BcmImpactAssessmentPojo>> listAll(
    ...
    @GET
    @Path("/{id}")
    public ResponseWrapper<BcmImpactAssessmentPojo> getById(@PathParam("id") String id) {
    ...
    @PUT
    @Path("/{id}")
    public ResponseWrapper<BcmImpactAssessmentPojo> update(@PathParam("id") String id, @Valid BcmImpactAssessmentDTO dto) {
