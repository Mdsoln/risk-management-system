...
const fundObjectiveSlice = createSlice({
    ...
    reducers: {
        ...
        getFundObjectivesSuccess(state, action: PayloadAction<PaginationResult<FundObjective>>) {
            state.loading = false;
            state.error = null;
...
export const fetchFundObjectives = (params: PaginationParams): AppThunk => async (dispatch) => {
    try {
